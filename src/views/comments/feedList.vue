<template>
    ...
    <div class="fixed2 task" @click.stop="_selfClose()">
        <div class="list-cont task-cont center" @click.stop>
            <div class="list-title task-title">饲料任务列表</div>
            <div class="task-nav flex-left">
                <div :class="'task-n-hide ' + (navtab==1?'task-n-act':'')" @click="_navChange(1)">饲料任务</div>
                <div :class="'task-n-hide ' + (navtab==2?'task-n-act':'')" @click="_navChange(2)">饲料购买</div>
            </div>
            <div v-show="navtab == 1" class="list-list task-nav-list list-scroll">
                <!-- 饲料 -->
                <template v-if="data && data.taskFeedList">
                    <div class="task-item" v-for="(item,index) in data.taskFeedList.task" :key="index">
                        <!-- 列表 -->
                        <div class="item2">
                            <div class="item-top">
                                <div class="top-name flex-algin-center"
                                    @click="_handleHidetips(index,navtab,data.taskFeedList.task[index])">
                                    {{item.taskname}} <i
            ...
            <!-- 购买  data.taskFeedList.TaskList_prop -->
            <div v-show="navtab == 2" class="list-list feed-buy task-nav-list">
                <template v-if="data && data.taskFeedList">
                    <div class="buy-tit" v-if="data.taskFeedList.chick">{{data.taskFeedList.chick[0].golds}}</div>
                    <div class="buy-list list-scroll">
                        <div class="buy-items task-item flex-bt flex-algin-center"
                            v-for="(item,index) in data.taskFeedList.shop" :key="index">
                            <div class="buy-left flex-left">
                                <div class="buy-imgs"><img :src="item.img" alt=""></div>
                                <div class="buy-msgs">
                                    <p>{{item.name}} <span v-show="item.explain != ''">{{item.explain}}</span></p>
                                    <p>{{item.description}}</p>
                                </div>
                            </div>
                            <div class="buy-right" @click="data._shopMethods('feed',item.id)"><img :src="item.priceimg"
                                    alt=""></div>
                        </div>
                    </div>
                </template>
